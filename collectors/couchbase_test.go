...
var fakeFullConfigForCouchbase Config
...
func init() {
	...
	fakeFullConfigForCouchbase = Config{
		AppName:        "test-newrelic-plugin",
		NewRelicKey:    "somenewrelickeyhere",
		DefaultDelayMS: 1000,
		Collectors: map[string]CommonConfig{
			"couchbase": CommonConfig{
				Enabled:         true,
				DelayMS:         500,
				CollectorConfig: couchbaseFakeConfig,
			},
		},
	}
}

func TestCouchbaseCollector(t *testing.T) {
	g := goblin.Goblin(t)
	...
	var tests = []struct {
	...
	}{
		{
			...
			ExpectedResult:  []map[string]interface{}{},
			TestDescription: "Successfully GET Couchbase stats",
		},
	}

	for _, test := range tests {
		g.Describe("TestCouchbaseCollector()", func() {
			g.It(test.TestDescription, func() {
				runner = test.HTTPRunner
				stats := make(chan []map[string]interface{}, 1)
				CouchbaseCollector(fakeFullConfigForCouchbase, stats)
				close(stats)
				for stat := range stats {
					g.Assert(len(stat)).Equal(3)
				}
			})
		})
	}
