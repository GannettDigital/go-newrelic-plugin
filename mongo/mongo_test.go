	...
	var tests = []struct {
	...
	}{
		{
			...
			InputSession: NewMockSession(
				MockSessionResults{
					DatabaseNamesResult: []string{"foo", "bar", "admin"},
				},
				map[string]MockDatabaseResults{
					...
					"admin": MockDatabaseResults{
						RunResult: []byte(replStatsData()),
					},
				},
				nil,
			),
...
func TestReadDBStats(t *testing.T) {
	g := goblin.Goblin(t)
...
func TestReadServerStats(t *testing.T) {
	...
	for _, test := range tests {
		g.Describe("readServerStats()", func() {
			g.It(test.TestDescription, func() {
...
func TestReadDBReplicaStats(t *testing.T) {
	g := goblin.Goblin(t)

	tests := []struct {
		InputLog        *logrus.Logger
		InputDB         MockDatabase
		ExpectedRes     bool
		ExpectedData    ReplStats
		TestDescription string
	}{
		{
			InputLog: logrus.New(),
			...
			TestDescription: "Should successfully read replica set stats",
		},
		{
			InputLog: logrus.New(),
			InputDB: MockDatabase{
				...
				Err: errors.New("not running with --replSet"),
			},
			ExpectedRes:     false,
			ExpectedData:    ReplStats{},
			TestDescription: "Should return false and no data, but not exit when replication is not enabled",
		},
	}

	for _, test := range tests {
		g.Describe("readDBReplicaStats()", func() {
			g.It(test.TestDescription, func() {
				res, data := readDBReplicaStats(test.InputLog, test.InputDB)

				g.Assert(res).Equal(test.ExpectedRes)
				g.Assert(data.Set).Equal(test.ExpectedData.Set)
				g.Assert(len(data.Members)).Equal(len(test.ExpectedData.Members))
			})
		})
	}
}
...
func replStatsData() string {
	return `{
		"set": "TykReplSet",
		"date": "2017-04-21T20:38:25.900Z",
		"myState": 1,
		"term": 57,
		"heartbeatIntervalMillis": 2000,
		"members": [{
				"_id": 0,
				"name": "dataTykReplSet_1.mongodb:27017",
				"health": 0,
				"state": 8,
				...
				"optimeDate": "1970-01-01T00:00:00Z",
				"lastHeartbeat": "2017-04-21T20:38:24.522Z",
				"lastHeartbeatRecv": "2017-03-14T16:11:47.860Z",
				"pingMs": 0,
			...
			{
				"_id": 1,
				"name": "dataTykReplSet_2.mongodb:27017",
				"health": 1,
				"state": 1,
				"stateStr": "PRIMARY",
				"uptime": 3299276,
				...
				"optimeDate": "2017-04-21T20:38:20Z",
				"electionTime": 1489507917,
				"electionDate": "2017-03-14T16:11:57Z",
				"configVersion": 3,
				"self": true
			},
			{
				"_id": 2,
				"name": "dataTykReplSet_3.mongodb:27017",
				"health": 1,
				"state": 2,
				"stateStr": "SECONDARY",
				"uptime": 3299264,
				...
				"optimeDate": "2017-04-21T20:38:20Z",
				"lastHeartbeat": "2017-04-21T20:38:25.690Z",
				"lastHeartbeatRecv": "2017-04-21T20:38:25.688Z",
				"pingMs": 0,
				"syncingTo": "dataTykReplSet_2.mongodb:27017",
				"configVersion": 3
			}
		],
		"ok": 1
